...
export function RiskDisclaimer({
  ...
  if (variant === "inline") {
    return <p className={cn("text-xs leading-relaxed text-muted-foreground", className)}>{body}</p>;
  }

  return (
    <aside
      className={cn("flex gap-3 rounded-lg border border-crimson/35 bg-crimson/8 p-4", className)}
    >
